...
**使用规则**
- **文字**：正文 `--n9`；次要 `--n7`（白底或 `--n1` 底）；三级/提示 `--n6`（**仅纯白底 `--n0`**）；更浅浅底（`--n2`~`--n4`）上的正文用 `--n8`。完整矩阵见下方。
...
**⚠️ 文字色 × 底色 达标矩阵（2026-09-15 实测，AA 正文需 ≥4.5:1）**

| 前景 \ 背景 | `--n0` 白 | `--n1` | `--n2` | `--n3` | `--n4` |
|---|---|---|---|---|---|
| `--n6` #6a758c | **4.63 ✓** | 4.39 ✗ | 4.16 ✗ | 3.84 ✗ | 3.56 ✗ |
| `--n7` #697386 | **4.78 ✓** | **4.53 ✓** | 4.29 ✗ | 3.96 ✗ | 3.68 ✗ |
| `--n8` #3d4657 | 9.49 ✓ | 9.00 ✓ | 8.54 ✓ | 7.87 ✓ | 7.31 ✓ |
| `--n9` #1f2a44 | 14.26 ✓ | 13.52 ✓ | 12.82 ✓ | 11.82 ✓ | 10.97 ✓ |

**据此确定的选择规则（必须遵守）**：
- **`--n6` 只用于纯白底（`--n0`/`--card`）**。它**唯一达标的底色就是白底**，故其定位是「白底上的三级文字 / 图标」：提示语、占位符、表格行序号、连接词、清空「×」、下拉箭头。
- **`--n7` 用于白底或 `--n1` 底**的次要文字（如页面底、卡片浅底的说明行）。落在 `--n1` 上时**必须用 `--n7`，不能用 `--n6`**。
- **落在 `--n2`/`--n3`/`--n4` 等更深浅底上的正文，一律用 `--n8`**（`--n6`/`--n7` 在这些底色上均不达标）。

**已知的两处边缘情形（评估后决定保留，理由如下）**
1. `.cell-inp:hover` 会把单元格底色变成 `--n1`，此时该格的 `::placeholder`（`--n6`）为 4.39:1。**属悬停瞬态**，且用户开始交互时占位符通常已被输入内容取代；改用 `--n7` 会与同格正文色混淆，故保留。
2. 同一悬停态下的 `.clear-btn`「×」是**图形**而非文本，按 WCAG 1.4.11 只需 3:1 —— 4.39:1 已达标。`.drp-arrow`/`.ms-arrow` 同理。

> 现网核查：现有 16 处 `--n6` 用法的容器背景经逐条回查 —— 卡片（`.aq-card`/`.sp-process-card`/`.ft-card-name`/`.stage-split-menu`）均为 `--n0` 白底，表格行序号与占位符在单元格白底上；**唯一落在 `--n1` 的是上述悬停瞬态**。故当前调用点全部合规或属上述豁免。
...
2. **对比度必须回查「真实背景」**。`--n6` 按白底算得 4.63:1 达标，但在 `--n1` 仅 4.39:1、`--n2` 4.16:1、`--n3` 3.84:1 —— **它唯一达标的底色就是白底**。（起初误算为「`--n1` 4.53:1 达标」，实为把 `--n7` 的值串到了 `--n6` 上。）据此在 §1 给出实测矩阵与逐级选用规则，并逐条回查现有 16 处调用的容器背景。
...
7. **「硬编码色」必须分两类，处置方向相反**（2026-09-17 补）：`var(--tok, #hex)` 的**兜底值**是已被令牌保护的安全网，替换它＝拆掉安全网；**直接写死**的才是真正绕过令牌、可无损收敛的部分。本次实测：218 处直接写死 vs 22 处兜底值 —— 混为一谈会得出完全错误的工作量。且 **≥3 处的值里多数并无等价令牌**（近似重复色），"按同值聚合就统一"会**改变颜色**，不是等价变换。
8. **回溯 CSS 声明边界时必须跳过注释**（2026-09-17 补）：注释里**没有分号**，会让「声明头」被注释文本污染 —— 曾把 `--n0: #ffffff` 判定为可替换，产出 `--n0: var(--n0)` **自引用**（整条令牌静默失效）。同时必须排除注释内的色值（说明性注释常引用色号）。**凡脚本要"替换某个 token 的值"，就要加 `--x: var(--x)` 自引用断言**。

### 8.3 执行状态（②③④⑤ 已于 2026-09-17 落地；① 保持现状）

> 落地详情与差异验证见 `output/UI83实施与差异验证报告_2026-09-17.md`（前端 `4841eee`）。

| 顺序 | 事项 | 状态 |
|---|---|---|
| 1 | 输入框边框对比度 | ⏸ **保持现状**（按决定不压深，保留轻量留白风格） |
| 2 | 自定义控件无障碍语义 | ✅ 已做（纯属性，零视觉影响） |
| 3 | 同值硬编码色（≥3 处）收敛 | ◐ **部分完成**：可证明无损的 62 处已收敛；剩余 14 个近似重复色需截图回归 |
| 4 | `--text` 定稿 | ✅ 已在 `:root` 定义 `--text: #222`，5 处死兜底清理 |
| 5 | toast 堆叠 / loading 收敛 / listTab | ✅ 已做（listTab 改 localStorage 持久化，见下） |

#### ① 的表述更正（2026-09-17）

原文只提 `--n5`（1.81:1），但**真正决定「输入框能否被看见」的是默认态 `--n4` `#dde2ec`（1.30:1）**；`--n5` 只出现在 `:hover`。把 hover 边框做达标而默认边框不达标 = 修了次要的、漏了主要的。

实测对比度（底色纯白，门槛 3:1）：`--n3` 1.21 · **`--n4` 1.30** · `--n5` 1.81 · `--line-accent` 2.35 · `--focus` 3.02 ✅ · `--n7` 4.78 ✅

另：`#8eaadb` 现为 `--line-accent`，**仅作装饰描边、非焦点指示** —— 按 1.4.11 的适用边界本就不在要求范围内。

#### ② 的两处实现取舍

- **`MultiSelect` 不用 listbox/option**：它是「N 个独立筛选开关」而非单选列表 → 用 `role=group` + `aria-pressed`，各选项保持**原生 `button`**（键盘天然可用，无需自造 tabindex 管理）。改用 listbox 反而要为按钮覆写角色并接管键盘。
- **`NameSuggest` 补了键盘导航**：此前只有 Escape。声明 `role=listbox` 却不给键盘操作等于半成品 —— 读屏用户被告知「有列表」却无法操作。Enter **仅在有高亮项时**接管，否则保持 textarea 原生换行。

#### ⑤-a 的偏离说明

原文写「`listTab` 入 URL」，实做改为 **localStorage 持久化**：

1. §9.3 把「一级页 = 裸路径（无 #、无查询串）」定为**对外契约**，带 `?tab=` 就破了它；
2. hash 模式下 `router.replace()` 发的是「仅片段」相对 URL、**改不了查询串**（见 §9.3 与 `App.vue` 注释），额外 `history.replaceState` 会与路由回收敛**竞争、互相覆盖**；
3. 页签在 SPA 会话内本就是响应式状态（前进/后退天然保留），唯一缺口是整页刷新。

→ 持久化拿到同一收益，**零契约风险、地址栏零可见变化**。

#### ③ 的剩余部分（待决策）

14 个近似重复色 / 156 处，最典型是 **danger 家族 5 个红**（`#fdecec` 10、`#f2cdcd` 8、`#b53a3a` 7、`#d92020` 4、`#fdeaea` 3）与既有 `--danger`(`#c0392b`) / `--danger-bg`(`#fdecea`) 并存 —— 合并会产生**可见的颜色变化**，需截图回归。

---

## 9. 分享与深链（交互契约，2026-09-15 定稿）

> 分享链接格式属于**对外契约**（接收方看到什么、点开后落在哪），改动前先确认本节。
> URL 参数放置与清理的工程规则见 `docs/CODING_STANDARD.md §12`。

### 9.1 两种分享的文案格式

| 入口 | 剪贴板内容 | 说明 |
|---|---|---|
| 一级页「分享本页」 | **仅裸链接**（`…/workpreparation-01/`） | 分享**项目列表**，不含任何项目 |
| 二级页「分享本页」 | `链接` + **换行** + `项目名称 · 准备单 · 各清单…` | 条目分隔符固定 `" · "` |

**二级页示例**（A 检项目）：

```
https://…/workpreparation-01/?pid=<项目 id>
B-1234 定检 · 工作准备单 · 工卡分配清单 · 航材清单 · 工具清单
```

**「准备单及各清单」按项目类型给出**（严格对齐二级页实际页签，勿自造）：

| 项目类型 | 说明部分 |
|---|---|
| A 检 | 项目名 · 工作准备单 · 工卡分配清单 · 航材清单 · 工具清单 |
| 单独项目 | 项目名 · 单项准备单 · 航材清单 · 工具清单 |
| 换发 / APU | 项目名 · 换发/APU 准备单 · 手册清单 · 串件航材清单 · 串件工具清单 |
| 零散 / 历史项目 | 项目名 · 工具清单（此类二级页只渲染工具清单） |

**隔断为什么用换行**：链接本体独占一行，任何聊天窗口都能整段识别为可点击链接，不受拼接字符干扰。

### 9.2 深链形态：必须是查询串 `?pid=<id>`

**用查询串而非 hash** —— CloudBase 测试域名的「风险提醒」中间页会把 hash 换成 `#/` 但保留查询串，而分享链接的**首次点击必然经过该中间页**。用 hash 会导致首次点击落到一级页（`docs/CLOUDBASE_DEPLOY.md §6`、`CODING_STANDARD.md §12.1`）。

用 **id** 而非名称/日期：id 不可变（改名后仍有效、同名同日不会串项目）。

### 9.3 地址栏收敛规则

| 场景 | 地址栏应呈现 |
|---|---|
| 通过 `?pid=<id>` 进入二级页 | 收敛为 `#/project/<id>`，**并清掉 `?pid=`** |
| 应用内进入二级页 | `#/project/<id>` |
| 回到一级页 | 裸路径（无 `#`、无查询串） |
| 深链指向不存在的项目 | 回一级页 + toast「未找到该项目，可能已被删除或尚未同步」+ 清参数 |

> **必须清参数**：否则刷新/再次分享会把「一级页形态的地址」传出去，导致接收方落到一级页。
> 清理**必须用 `history.replaceState` 显式给出完整 URL**（hash 模式的 `router.replace()` 会保留查询串），理由见 `CODING_STANDARD.md §12.2`。

### 9.4 兼容性

- 旧格式 `?p=名称/创建日期`（历史分享链接）：保留解析，仅兼容、**不再生成**。
- 旧格式 `#/project/<id>`（hash 深链）：**应用内仍完整支持**；但**首次点击**（未经中间页的浏览器）仍会被中间页吞掉 —— 已发出的旧链接无法补救，根治靠绑定自定义域名（ICP 备案后中间页消失），届时 `?pid=` 格式继续可用、**无需改回 hash**。

---

## 10. 保存语义与临时实体（交互契约，2026-09-24 定稿）

> 本节约束**「保存」这个动作在不同状态下目标到底是什么**。改动保存相关逻辑前先确认本节。
> 工程实现见 `docs/CODING_STANDARD.md`；本节的变更记录见 `PROJECT_HANDOVER_2026-09-06.md §11`。

### 10.1 一条通用规则：临时实体上不放「保存」

模板库「编辑 / 新增模板」会新建一条**临时项目**承载编辑内容，并在关闭子页时**自动删除、不保存**
（`editingTemplateProjectId`）。在这类「关闭即删」的临时实体上，「保存」是个无意义的动作 ——
推上去的记录很快会被删掉，更糟的是**用户会以为改动已保存，而真正的目标（模板）并没有更新**。

**规则**：凡是「关闭即删」的临时实体，它页面上的「保存」必须指向**用户真正想保存的那个东西**，
而不是这个临时壳。
...
| 状态 | 「保存」的行为 |
|---|---|
| 模板编辑页（`isEditingTemplateProject`） | 打开**「保存模板」弹窗** —— 目标是模板，由用户确认覆盖哪一个 |
| 普通项目 / A 检 / 单独项目 | 不变：强制推送后台（保存该项目） |

配套约束：

- 「保存」按钮的 `title` 必须反映当前语义 —— 模板编辑态显示「模板编辑中：保存即写回模板」，
  否则显示「强制推送后台」。**状态变了而提示没变，用户就无从判断点了会发生什么。**
- 模板编辑入口在子页里（换发/APU → 甘特；单独项目 → 单项准备单）。若用户停在航材/工具清单等
  子页，那些组件未挂载 → 点「保存」时须**先自动切回编辑子页**再弹窗。

### 10.3 临时实体的回收（本地台账 + 启动清理，2026-09-24 落地）

`editingTemplateProjectId` 只在内存 —— 刷新后该临时项目不再被识别为模板编辑页，也就不会被自动
删除，会作为普通项目留在列表里。根因是 **`backToList()` 是唯一回收路径**：刷新 / 关标签页 /
编辑中途跳走都不会走它。

**做法**：模板编辑页创建临时项目后把 id 记进本地台账（`localStorage: toolbox.tplOrphans`，
**数组**且上限 10 —— 两个标签页并发编辑时单值会互相覆盖、必留一个孤儿），下次启动逐条清理。

| 阶段 | 行为 |
|---|---|
| 创建（`mode="edit"`） | `await createProject()` **之后**登记（云端可用时 id 由服务端 POST 返回，创建前拿不到） |
| 点「← 返回」 | `backToList()` 删除云端记录后**注销**台账（不留到下次启动） |
| 刷新 / 关页 / 中途跳走 | 走不到 `backToList` → 台账保留 → **下次启动清理**（云端 + 本地列表 + 注销） |
| 清理时该 id 已不存在 | 404 视为「已删除」→ 正常注销；网络异常则**保留台账**待下次重试 |

**安全边界（本机制唯一可能造成破坏的地方）**：只删台账里**精确记录过的 id**，不做任何特征猜测
（不按名字、不按类型、不按时间）—— 台账只在模板编辑入口写入，写进去的是刚由服务端返回的 id
（服务端 `uuid4().hex` 生成，不会与既有项目撞车），因此**不可能误删用户的真实项目**。
台账读写全部 `try/catch`：存储不可用时静默退化为「不清理」，不影响主流程。

**刷新时的视图处理**：刷新瞬间地址栏正停在 `#/project/<临时项目>`，深链恢复会把它设成「当前打开的
项目」→ 清理后**一并退回一级页**，避免详情页指向一条已不存在的记录。

**已知局限（如实标注）**：

- **换设备 / 换浏览器 / 清缓存** → 台账不在本机，清不掉（要覆盖此场景需改用服务端标记方案）。
- **离线启动** → 整轮跳过并保留台账，等下次在线再清。
- **两个标签页同时处于模板编辑态** → 后启动的一页可能清掉另一页正在编辑的临时项目
  （仅表现为那一页保存时报错；模板内容本身不受影响）。
- **用户把孤儿「收养」为真实项目** → 登记到下次启动之间若主动打开那条记录并投入内容，
  下次启动会一并清掉（窗口期 = 从刷新到下次页面加载）。
